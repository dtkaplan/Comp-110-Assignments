...
econForm =
  ScorecardSmall %>%
  select(INSTNM, CONTROL, ADM_RATE ,NPT41_PUB, NPT43_PUB, NPT45_PUB, NPT41_PRIV,  NPT43_PRIV,  NPT45_PRIV) %>%
  filter(CONTROL != 3) %>%
  gather(key = incomeLevel, value = netCost, NPT41_PUB, NPT43_PUB,  NPT45_PUB, NPT41_PRIV, NPT41_PRIV, NPT43_PRIV, NPT45_PRIV) 
```
...
```{r }
ggplot(data=econFormQs,aes(x=ADM_RATE,y=netCost))+geom_point()+facet_wrap(short_name~CONTROL) + stat_smooth(method=loess) #not as pretty, but almost the same result
