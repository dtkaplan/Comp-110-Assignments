...
To be a tidy table, each column needs to be a variable containing the same type of value for each case. Number of states is a quantitative variable. However, 'it depends' is not a number, which differs it to other values. Also, the format for values should be the same. '1837 to 1841' should be '1837-1841'. 'Lincoln, Abraham' should be 'Abraham Lincoln'.
...
3. faa is a categorical variable. name is categorical. lat is catagorical. alt is quantitative. tz is categorical. dst is categorical.
...
1) a data frame
```{r}
fireplace <- BabyNames %>% filter(sex == "F")
```
...
```{r}
fireplace <- (result=sum(4,5))
```

4) a variable
```{r}
fireplace <- BabyNames %>% filter(name=="Mike")
```
...
c) summarizes the mean other than the sum.
